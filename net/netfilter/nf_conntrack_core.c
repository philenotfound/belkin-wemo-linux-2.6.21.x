...
#define CONNTRACK_PORT_ARRAY_SIZE	11
int general_traffic_conntrack_max = 200;
static __u16 privilege_conntrack_port[CONNTRACK_PORT_ARRAY_SIZE]={80,23,3128,1863,21,25,110,443,53,67,68};
#endif
	       ...
	       unsigned int dataoff)
{
	...
	//printk(KERN_EMERG "nf_conn_max=[%d], g_conn_max=[%d], nf_conn_count=[%d]\n",
//				nf_conntrack_max, general_traffic_conntrack_max, atomic_read(&nf_conntrack_count));
#ifdef CONFIG_PRIVILEGE_CONNTRACK
//printk(KERN_EMERG "skb->nh.iph->protocol=[%d], port1=[%d]\n", skb->nh.iph->protocol, __swab16(*((unsigned short *)(skb->nh.iph)+11)));
	/* Here we examine by port if the traffic is allowed to pass. */
	if(general_traffic_conntrack_max &&
		atomic_read(&nf_conntrack_count) >= general_traffic_conntrack_max)
	{
//printk(KERN_EMERG "check ports\n");		
		/* check ports */
		int i, passable = 0;
...
//			printk(KERN_EMERG "skb->nh.iph->protocol=[%d], port1=[%d]\n", skb->nh.iph->protocol, __swab16(*((unsigned short *)(skb->nh.iph)+11)));
		...
		{
			printk(KERN_WARNING
				       "ip_conntrack: general conntrack streams full, dropping"
				       " packet.\n");
			//printk(KERN_EMERG "Drop!!\n");
			return ERR_PTR(-ENOMEM);
		}
	}
//printk(KERN_EMERG "Pass!!\n");

#endif /* CONFIG_PRIVILEGE_CONNTRACK */
