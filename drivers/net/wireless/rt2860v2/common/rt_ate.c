	...
	IN  struct ate_racfghdr *pRaCfg)
{
	...
	pAdapter->ate.bQAEnabled = TRUE;
	DBGPRINT(RT_DEBUG_TRACE,("pAdapter->ate.bQAEnabled = %s\n", (pAdapter->ate.bQAEnabled)? "TRUE":"FALSE"));
        ...
        IN PRTMP_ADAPTER pAd)
{
	...
	if (pAd->ate.bQATxStart == FALSE) 
	{
		...
		/* Carrier Suppression set BBP R24 bit0=1 */
                ATE_BBP_IO_READ8_BY_REG_ID(pAd, BBP_R24, &BbpData);
               BbpData |= 0x00000001; //set bit0=1
		ATE_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R24, BbpData);
...
#else
	DBGPRINT(RT_DEBUG_OFF, ("ATE=APSTOP, ATESTART, TXCONT, TXCARR, TXCARS, TXFRAME, RXFRAME\n"));
#endif // CONFIG_RT2880_ATE_CMD_NEW //
#ifdef HW_ANTENNA_DIVERSITY_SUPPORT
	...
	IN  struct ate_racfghdr *pRaCfg)
{
	...
	pAdapter->ate.bQARxStart = TRUE;
	Set_ATE_Proc(pAdapter, "ANTDIVCBA");
...
{    
...
#ifdef RT3350
	if (!IS_RT3350(pAd) || !(pAd->TxPowerCtrl.bInternalTxALC))                  
#else
	...
	{                              
		...
		ATE_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R49, BBP49Value);     
	}
