...
UINT32 TSSIRatioDot85(struct _RTMP_ADAPTER *pAd, INT32 delta_power)/* TSSI update */
{
	UINT32 ratio = 0;

	DBGPRINT(RT_DEBUG_OFF, ("---> %s\n", __FUNCTION__));/* TSSI update */
...
UINT32 TSSIRatioDot7(struct _RTMP_ADAPTER *pAd, INT32 delta_power)/* TSSI update */
{
	UINT32 ratio = 0;
	...
	IN PRTMP_ADAPTER		pAd)
{
	...
	RT28xx_EEPROM_READ16(pAd, (EEPROM_TSSI_STEP_OVER_2DOT4G - 1), E2PValue);/* TSSI update */

	/* bit 2 of 0x77 is 1, select updated internal TSSI ratio table */
	...
	IN PRTMP_ADAPTER			pAd)
{
	...
	RT28xx_EEPROM_READ16(pAd, (EEPROM_OFDM_MCS6_MCS7 - 1), TxPowerOFDM54);
        ...
        RT5350_desiredTSSIOverHT40[MCS_5] = TSSIBase * RTMP_CHIP_ASIC_GET_TSSI_RATIO(pAd, TxPower - TxPowerOFDM54 + TSSI_x.delta - 1);
        ...
        RT5350_desiredTSSIOverHT40[MCS_6] = TSSIBase * RTMP_CHIP_ASIC_GET_TSSI_RATIO(pAd, TxPower - TxPowerOFDM54 + TSSI_x.delta - 1);
        RT5350_desiredTSSIOverHT40[MCS_7] = TSSIBase * RTMP_CHIP_ASIC_GET_TSSI_RATIO(pAd, TxPower - TxPowerOFDM54 + TSSI_x.delta - 1);
