...
#define RTMP_STA_STANDARD_IOCTL_HANDLE(__pAd, __pData, __Data, __SubCmd)	\
	case CMD_RTPRIV_IOCTL_STA_SIOCGIWNAME:									\
		return RtmpIoctl_rt_ioctl_giwname(__pAd, __pData, __Data);			\
	...
	case CMD_RTPRIV_IOCTL_STA_IW_SET_PARAM_PRE:								\
		return RtmpIoctl_rt_ioctl_setparam(__pAd, __pData, __Data);			\
...
{
	if (INFRA_ON(pAd) || ADHOC_ON(pAd))
		memcpy(pData, &pAd->CommonCfg.Bssid, ETH_ALEN);
#ifdef WPA_SUPPLICANT_SUPPORT    
	// Add for RT2870
	else if (pAd->StaCfg.WpaSupplicantUP != WPA_SUPPLICANT_DISABLE)
		memcpy(pData, &pAd->MlmeAux.Bssid, ETH_ALEN);
#endif // WPA_SUPPLICANT_SUPPORT //
	else
		return NDIS_STATUS_FAILURE;
	return NDIS_STATUS_SUCCESS;
