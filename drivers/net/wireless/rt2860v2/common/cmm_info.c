...
*/
//RAJEEV
#define HIDDEN_LINE_LEN (3)
#define	LINE_LEN	(4+33+20+23+9+8+7+3+HIDDEN_LINE_LEN)	// Channel+SSID+Bssid+Security+Signal+WiressMode+ExtCh+NetworkType
#ifdef CONFIG_STA_SUPPORT
#ifdef WSC_STA_SUPPORT
	...
	IN  UINT32		MsgLen)
{
	...
	sprintf(msg+strlen(msg), "%-23s", SecurityStr);
		...
		//Hidden - Rajeev
		if(pBss->Hidden)
			sprintf(msg+strlen(msg),"%-3s", "1");
		else
			sprintf(msg+strlen(msg),"%-3s", "0");
	...
	IN	RTMP_IOCTL_INPUT_STRUCT	*wrq)
{
	...
	sprintf(msg,"%s","\n");
	sprintf(msg+strlen(msg),"%-4s%-33s%-20s%-23s%-9s%-8s%-7s%-3s%-3s\n",
	    "Ch", "SSID", "BSSID", "Security", "Siganl(%)", "W-Mode", " ExtCH"," NT"," H");	
	...
	for(i=0; i<pAdapter->ScanTab.BssNr ;i++)
	{
		...
		DBGPRINT(RT_DEBUG_TRACE, ("RTMPIoctlGetSiteSurvey - Channel-%d,SSID-%s, Hidden-%d\n",pBss->Channel,pBss->Ssid,pBss->Hidden));
		if( pBss->Channel==0)
			break;

//		if((strlen(msg)+max_len ) >= IW_SCAN_MAX_DATA)
		if((strlen(msg)+100 ) >= TotalLen)
			break;
	...
	IN	ULONG			BufLen)
{
	...
	IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
		snprintf(pBuf, BufLen, "\t%s", pAd->ApCfg.MBSSID[pObj->ioctl_if].Ssid);
#endif // CONFIG_AP_SUPPORT //

#ifdef CONFIG_STA_SUPPORT
	IF_DEV_CONFIG_OPMODE_ON_STA(pAd)
		snprintf(pBuf, BufLen, "\t%s", pAd->CommonCfg.Ssid);
#endif // CONFIG_STA_SUPPORT //
	return 0;
}
