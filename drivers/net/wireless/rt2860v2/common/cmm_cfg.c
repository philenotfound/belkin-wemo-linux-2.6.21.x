	...
	IN	PSTRING			arg)
{
		...
		if (value > 1)
	{
		...
		if (value == 3)
			...
			BBPValue = 0xa3; /* default Main Ant */
	...
	IN	ULONG					Data)
{
	...
	switch(cmd)
	{
		...
		case CMD_RTPRIV_IOCTL_CFG80211_CFG_INIT:
		{
			RT_CMD_CFG80211_CONFIG *pConfig = (RT_CMD_CFG80211_CONFIG *)pData;

			pAd->pCfgDev = pConfig->pCfgDev;
			pAd->CFG80211_Register = pConfig->CFG80211_Register;
		}
			break;
